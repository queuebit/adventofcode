...
var craneOps9000 = function (stacks, instructions) {
...
var craneOps9001 = function (stacks, instructions) {
    instructions.forEach(function (instruction) {
        var regexpInstruction = /move (\d+) from (\d+) to (\d+)/;
        var match = instruction.match(regexpInstruction);
        if (match) {
            ...
            var fromStackID = fromStack - 1;
            var toStackID = toStack - 1;
            var cratesInFromStack = stacks[fromStackID].length;
            var moveFromStackHeight = cratesInFromStack - nCrates;
            var unmovedCrates = stacks[fromStackID].slice(0, moveFromStackHeight);
            var movedCrates = stacks[fromStackID].slice(moveFromStackHeight);
